...
test('AuthenticationTokenMiddleware Success', async () => {
  const tokenStore = new Mock<AuthenticationTokenStore>()
    .setup(m => m.retrieveToken())
    .returns(null)
    .setup(m => m.storeToken(It.IsAny<AuthenticationToken | null>()))
    .returns();
  const tokenAccessor = new Mock<AuthenticationTokenAccessor<any>>()
    .setup(m => m.requestToken(It.IsAny<any>(), It.IsAny<string | undefined>()))
    .returnsAsync({
      token: goodToken,
    ...
    .setup(m => m.validateToken(It.IsAny<AuthenticationToken | null>()))
    .returnsAsync(false);
  ...
  const client = wretch(mockUrl).polyfills({ fetch }).accept('application/json').middlewares([middleware]);
...
test('AuthenticationTokenMiddleware 401 then 200', async () => {
  ...
  const tokenAccessor = new Mock<AuthenticationTokenAccessor<any>>()
    ...
    .setup(m => m.validateToken(It.Is<AuthenticationToken>(token => token.token === oldToken)))
    .returnsAsync(false)
    .setup(m => m.validateToken(It.Is<AuthenticationToken>(token => token.token === goodToken)))
    .returnsAsync(true);
  const middleware = authenticationTokenMiddleware({
    credentialProvider: () => {},
...
test('AuthenticationTokenMiddleware fails', async () => {
  ...
  const tokenAccessor = new Mock<AuthenticationTokenAccessor<any>>()
    .setup(m => m.requestToken(It.IsAny<any>(), It.IsAny<string | undefined>()))
    .returnsAsync(null)
    .setup(m => m.validateToken(It.IsAny<AuthenticationToken>()))
    .returnsAsync(false);
  const middleware = authenticationTokenMiddleware({
    credentialProvider: () => {},
    tokenStore: tokenStore.object(),
...
test('AuthenticationTokenMiddleware refresh token', async () => {
  ...
  const tokenAccessor = new Mock<AuthenticationTokenAccessor<any>>()
    .setup(m =>
      m.requestToken(
        It.IsAny<any>(),
        It.Is<string | undefined>(s => s === goodRefreshToken)
      )
    )
    .returnsAsync({
      token: goodToken,
...
test('AuthenticationTokenMiddleware existing valid token', async () => {
  ...
  const tokenAccessor = new Mock<AuthenticationTokenAccessor<any>>().setup(m => m.validateToken(It.IsAny<AuthenticationToken>())).returnsAsync(true);
  const middleware = authenticationTokenMiddleware({
    credentialProvider: () => {},
    tokenStore,
    tokenAccessor: tokenAccessor.object(),
